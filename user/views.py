...
def signup(request):
    if request.method == "POST":
        ...
        if User.objects.filter(email=email).exists():
            return render(
                request, "user/signup.html", {"error": "Email already exists."}
            )
        ...
        if not password1 == password2:
            return render(
                request, "user/signup.html", {"error": "passwords do not match."}
            )
...
def signin(request):
    if request.method == "POST":
        ...
        try:
            ...
            return render(
                request, "user/login.html", {"error": f"No account found with: {email}"}
            )
        ...
        if not user.is_active:
            return render(
                ...
                "user/login.html",
                {
                    "error": "Mail has been sent to you with verification link, Please verify and try again!"
                },
            )
...
def forgot_password(request):
    if request.method == "POST":
        ...
        try:
            ...
            return render(
                request, "user/forgot_password.html", {"error": "Email does not exist!"}
            )
...
def password_reset(request, id, token):
    ...
    if request.method == "POST":
        ...
        if p1 == p2:
            ...
            return render(
                request, "user/reset_password.html", {"error": "Passwords do not match"}
            )
...
def resend_verification_mail(request):
    if request.method == "POST":
        ...
        if user.is_active:
            return render(
                request,
                "user/resend_verification_mail.html",
